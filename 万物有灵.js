  ...
  async _flush() {
    ...
    try {
      ...
      const res = await fetch(`${this.config.endpoint}/api/report`, {
        ...
        body: JSON.stringify({ batch, source: 'wanwu_plugin', version: '4.5' })
      });
      if (!res.ok) {
    ...
    try {
      ...
      switch (patch.scope) {
        ...
        case 'skills':
          ...
          break;
        case 'items': {
          const itemPayload = payload.items || payload;
          if (itemPayload && typeof itemPayload === 'object' && typeof ITEMS !== 'undefined') {
            applyDefinitionPatch(ITEMS, itemPayload, sanitizeItemDefinition);
            changed = true;
          }
          break;
        }
...
const GAME_TIPS = [
  ...
  'ℑ 育种有机会继承父母的优秀基因，适合培养高潜力后代。',
  'ℑ 进化通常需要等级、好感或材料条件，缺一项都不会触发。',
  'ℑ 商店商品会随地点变化，旅行到不同城镇可以发现不同食物。',
  ...
  'ℑ 战斗前先恢复生命和精力，残血出战很容易翻车。',
  'ℑ 多准备几种属性的宠物，面对克制关系时会更从容。',
  'ℑ 普通食物适合日常培养，稀有食物更适合冲刺关键成长。',
  'ℑ 捕捉失败不一定亏，观察目标血量和状态后再尝试会更稳。',
  'ℑ 队伍里保留一只耐久宠，可以在高难战斗中承担更多风险。',
  'ℑ 新获得的宠物先查看性格、天赋和技能，再决定培养方向。',
  'ℑ 资源有限时，优先培养一到两只主力宠物通常收益更高。',
  'ℑ 参加活动前记得清理背包空间，避免奖励领取时手忙脚乱。',
...
function getPlayerRace(player) {
  return PLAYER_RACES[player?.race || '精灵族'] || null;
}
