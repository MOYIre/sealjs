...
// @license     Apache-2
// @updateUrl   https://fastly.jsdelivr.net/gh/MOYIre/sealjs@main/万物有灵-市场.js
// ==/UserScript==
...
// ==================== 配置 ====================
const CONFIG = {
  maxListings: 5,        // 每人最多挂售数量
  taxRate: 0.05,         // 交易税率 5%
  minPrice: 50,          // 最低价格
  maxPrice: 99999,       // 最高价格
  listingExpire: 7 * 24 * 60 * 60 * 1000, // 挂售过期时间 7天
};
...
function formatPet(pet) {
  const main = getMain();
  const r = main ? { '普通': '', '稀有': '★', '超稀有': '★★', '传说': '★★★' }[pet.rarity] : '';
  const e = pet.element ? `[${pet.element}]` : '';
  return `${r}${e} ${pet.name} Lv.${pet.level} 战力:${Math.floor(pet.atk * 2 + pet.def * 1.5 + pet.hp * 0.5)}`;
}
...
function init() {
  ...
  main.registerCommand('市场', (ctx, msg, p) => {
    ...
    if (!listings.length) return p.reply('【宠物市场】\n暂无宠物出售\n使用 .宠物 挂售 <仓库编号> <价格> 出售宠物');
    
    const lines = ['【宠物市场】', `当前在售: ${listings.length}只`, ''];
    
    listings.slice(0, 10).forEach(([id, item]) => {
      const petStr = formatPet(item.pet);
      lines.push(`#${id.slice(-4)} ${petStr}`);
      lines.push(`   💰 ${item.price}金 卖家: ${item.sellerName}`);
    });
  ...
  main.registerCommand('挂售', (ctx, msg, p) => {
    ...
    if (isNaN(storageIdx) || isNaN(price)) {
      return p.reply('用法: .宠物 挂售 <仓库编号> <价格>\n例: .宠物 挂售 1 500');
    }
    
    if (price < CONFIG.minPrice || price > CONFIG.maxPrice) {
      return p.reply(`价格范围: ${CONFIG.minPrice} - ${CONFIG.maxPrice} 金币`);
    }
    ...
    if (myListings.length >= CONFIG.maxListings) {
      return p.reply(`每人最多挂售${CONFIG.maxListings}只宠物`);
    }
    ...
    marketData.listings[listingId] = {
      ...
      price,
      sellerId: p.uid,
      sellerName: msg.sender.nickname || msg.sender.userId,
      time: Date.now(),
      expire: Date.now() + CONFIG.listingExpire,
    };
    saveMarket();
    
    p.reply(`已将 ${pet.name} 挂售\n价格: ${price} 金币\n挂售ID: #${listingId.slice(-4)}`);
    return seal.ext.newCmdExecuteResult(true);
  }, '挂售宠物', MOD_ID);
  ...
  main.registerCommand('购买宠物', (ctx, msg, p) => {
    ...
    // 放入仓库或队伍
    if (mainData.pets.length < main.Config.maxPets) {
      mainData.pets.push(item.pet);
    } else {
  ...
  main.registerCommand('撤销挂售', (ctx, msg, p) => {
    ...
    const mainData = main.DB.get(p.uid);
    if (mainData.storage.length >= main.Config.maxStorage) {
      return p.reply('仓库已满，无法返还宠物');
    }
  ...
  main.registerCommand('我的挂售', (ctx, msg, p) => {
    ...
    myListings.forEach(item => {
      const petStr = formatPet(item.pet);
      lines.push(`#${item.id.slice(-4)} ${petStr}`);
      lines.push(`   💰 ${item.price}金`);
    });
...
function waitForMain(callback, maxAttempts = 10) {
  ...
  if (maxAttempts <= 0) { console.log('[万物有灵-市场] 主插件未找到'); return; }
