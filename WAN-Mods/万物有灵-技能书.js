...
function init() {
  ...
  main.registerMod({
    id: MOD_ID, name: '万物有灵-技能书', version: '1.0.0', author: '铭茗',
    description: '技能书系统', dependencies: ['wanwu-ext'],
  });
  ...
  main.registerCommand('技能书', (ctx, msg, p) => {
    ...
    items.forEach(([name, count]) => {
      ...
      lines.push(`[${book.rarity}] ${name} x${count} ${element}`);
  ...
  main.registerCommand('学习技能', (ctx, msg, p) => {
    ...
    if (pet.skills.includes(book.skill)) return p.reply(`${pet.name}已经学会了${book.skill}`);
    pet.skills.push(book.skill);
    data.books[bookName]--;
    main.DB.save(p.uid, mainData);
    DB.save(p.uid, data);
    p.reply(`[学习] ${pet.name} 阅读了 ${bookName}\n[*] 学会了 ${book.skill}！`);
    return seal.ext.newCmdExecuteResult(true);
  }, '使用技能书', MOD_ID);
  ...
  main.registerCommand('给予技能书', (ctx, msg, p) => {
    ...
    if (!bookName) {
      ...
      return p.reply(`用法: .宠物 给予技能书 <技能书名>\n可用: ${books}...`);
    }
    if (!SKILL_BOOKS[bookName]) return p.reply('未知技能书');
    const data = DB.get(p.uid);
    data.books[bookName] = (data.books[bookName] || 0) + 1;
    DB.save(p.uid, data);
